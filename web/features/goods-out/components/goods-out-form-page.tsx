...
import { useCreateGoodsOut, useUpdateGoodsOut, useGoodsOutReceipt } from '../hooks/use-goods-out';
...
interface GoodsOutFormPageProps {
  receiptId?: string;
}

export function GoodsOutFormPage({ receiptId }: GoodsOutFormPageProps) {
  const router = useRouter();
  const { data: inventory } = useInventory();
  const { data: receipt, isLoading: isLoadingReceipt } = useGoodsOutReceipt(receiptId || "");
  ...
  const isPending = createGoodsOut.isPending || updateGoodsOut.isPending || isSubmitting || isLoadingReceipt;
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold">
                {isEditMode ? 'Edit Goods Out Receipt' : 'Create Goods Out Receipt'}
              </h1>
              <p className="text-sm text-muted-foreground">
                {isEditMode ? 'Update goods out receipt information' : 'Record goods sold to an account'}
              </p>
            </div>
          </div>
        </div>
